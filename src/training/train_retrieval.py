...
if __name__ == '__main__':
    ...
    model = MovielensModel(
                           layer_sizes = [config.model_config.model_params['ret']['layer1'], config.model_config.model_params['ret']['layer2']],
    ...
    # Results
    results = {}
    results['factorized_top_k/top_100_categorical_accuracy'] = history.history['factorized_top_k/top_100_categorical_accuracy']
    results['val_factorized_top_k/top_100_categorical_accuracy'] = history.history['val_factorized_top_k/top_100_categorical_accuracy']
    ...
    scann_index.index_from_dataset(
      tf.data.Dataset.zip((movies.map(lambda x: x["movie_title"]).batch(100),
                           movies.map(lambda x: x["movie_title"]).batch(100).map(model.candidate_model)))
    )
