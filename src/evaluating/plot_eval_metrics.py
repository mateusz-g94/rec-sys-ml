...
def plot_metric(js, model):
    ...
    ax[0].hlines(js['test_set'][metric], xmin = 0, xmax = len(js[metric]), colors = 'g', linestyle = '--', label = 'test')
    ax[0].set_title(metric)

    ax[1].plot(js['loss'], label = 'train')
    ax[1].plot(js['val_loss'], label = 'val')
    ax[1].hlines(js['test_set']['loss'], xmin = 0, xmax = len(js[metric]), colors = 'g', linestyle = '--', label = 'test')
    ax[1].set_title('loss')
