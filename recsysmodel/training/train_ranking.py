...
from recsysmodel.model.ranking import MovielensModel
...
if __name__ == "__main__":
    ...
    ratings_train = tf.data.experimental.CsvDataset(
        ...
        lambda x1, x2, x3: OrderedDict(
            {"user_id": x1, "movie_title": x2, "user_rating": x3}
        )
    )
    ...
    unique_movie_titles = np.unique(
        np.concatenate(list(ratings_train.map(lambda x: x["movie_title"]).batch(1000)))
    )
    ...
    model = MovielensModel(
        layer_sizes=[
            config.model_config.model_params["ran"]["layer1"],
            config.model_config.model_params["ran"]["layer2"],
        ],
    ...
    results["val_root_mean_squared_error"] = history.history[
